...
// TestMain makes this test binary a valid sandbox-shim host. Binding a session to
// an external agent brings up an instance through agenthost.buildAgentCmd →
// libsandbox.Command, which re-exec's /proc/self/exe as the sandbox shim to build
...
// of confining the agent, and every external-agent test would see the downstream
