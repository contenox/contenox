...
The tree is developed in the maintainer's monorepo and published here by
**source drop**. Every commit on `main` is one release — `Release vX.Y.Z`,
carrying the complete tree — and every tag is a signed release that
`.github/workflows/release.yml` turns into binaries and a GitHub release. There
is no development branch here, no merge queue, and nothing reaches this
repository by any path other than a drop. What you clone is the whole server,
Apache-2.0, the same code the maintainer runs; what you do not see is the
commit-by-commit history between two releases.

What that means for contributing, stated plainly:

- **Issues are the front door.** Bugs, rough edges, questions, ideas and docs
  gaps each have a template; [SUPPORT.md](SUPPORT.md) says which to pick.
  Security reports go by mail, never as an issue — [SECURITY.md](SECURITY.md).
- **Pull requests are welcome, and are applied upstream by hand.** A maintainer
  carries your diff into the monorepo, credits you as `Co-authored-by` on the
  release commit and names the change in the release notes, and it appears
  here with the next drop — not as your commit merged onto a branch. Keep a PR
  small and self-contained. For anything larger than a fix, open a
  *Contribution Proposal* issue first so the work is agreed before it is
  written.
- **No branch to track.** Fork and build from any release commit or tag; when
  the next release lands, rebase onto it. Your PR branch is the evidence of the
  change, not its delivery vehicle, and the PR is closed once the change has
  shipped.
- **Licensing.** Apache-2.0, inbound equals outbound: by opening a pull request
  you license the contribution under this repository's license. No CLA, no
  sign-off ritual.

## Code of Conduct

[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md). Keep technical disagreements
...
```text
CLI / beam / ACP stdio sessions / relay
    ->
...
integrations and the terminal client are adapters around that runtime — they
must not re-create chain semantics elsewhere.

### The product surface

- `contenox beam` — the terminal client, and what bare `contenox` opens on a TTY
...
existed — each time because a change landed without it. `task test-cli-help`
pins the help against a golden script.
...
OpenAPI-backed remote tool the operator attaches. HITL policy wraps tool
execution where approval is required.
...
| `internal/surfaces/beam/` | the terminal client, an ACP client over an in-process bridge |
| `internal/kernel/agentinstance/` | the embeddable fleet kernel (missions run in-process) |

## Repository structure

The `contenox` binary is the only entrypoint; `contenox --help` lists its
commands, and the help is pinned by `task test-cli-help`.

The layout is layered, and the domain logic lives under Go `internal/` so
the compiler defines the public surface. The top-level `lib*` packages are the
deliberate exceptions — `libacp` (a reusable Go ACP implementation),
`librelay` (the runtime↔relay wire contract), and the infrastructure leaves
`libdbexec`, `libbus`, `libkvstore`, `libtracker`, `libevents`, `libcipher`,
`libroutine`, `liblease`, `liblog` and `errdefs` — all importable as
...
internal/services/       domain services: chat, session, mission, fleet,
                         hitl, mcp, tools, vfs, agent declarations, …
...
internal/surfaces/       thin adapters: contenoxcli (CLI), acpsvc (ACP
                         sessions), beam (terminal client), fleetboot
                         (mission host bootstrap)
internal/relay*/         the outbound connector to a relay: link, acp
                         tunnel, pairing, credentials, test double
internal/libsandbox/     sandboxing, no LLM dependency
lib*/, errdefs/          top-level libraries with no LLM dependency
schema/                  published JSON Schemas, generated from the Go types
...
Releases are not cut here. What happens when a tag lands is documented in
...
1. Open an issue first for anything beyond a fix — a *Contribution Proposal*
   for features and refactors, a *Bug Report* for a defect you intend to fix.
2. Branch from the latest release commit on `main`; any branch name is fine.
   The branch is evidence, not the delivery vehicle (see the top of this page).
3. One change per pull request, with a commit message that says what and why.
   The PR template asks what you checked; answer it honestly, including
